...
<div class="w-11/12 m-auto relative bg-white page-fix shadow sm:shadow-md md:shadow-lg lg:shadow-xl p-4 md:p-8">
    ...
    <?php 
        if(! is_page( )){
            ?>
                <a class="fade-in-bottom text-sm text-gray-500 hover:text-red-700  md:text-base transition ease-in-out duration-200" href="<?php echo get_home_url( ).'/author/'. get_the_author_meta( 'user_login' ) ?>"><i class="fas fa-user mr-2"></i>by <?php echo get_the_author_meta( 'display_name' ) ?></a>
